...
try:
    _posproc_key = importlib.import_module("posproc.key")
    Key = _posproc_key.Key
    Random_Key_Generator = _posproc_key.Random_Key_Generator
except Exception:
    class Key:
        def __init__(self, key_as_str):
            self._key_as_str = key_as_str
            self._size = len(key_as_str)

        def __str__(self):
            return self._key_as_str

    def Random_Key_Generator(key_size, seed=None):
        rng = random.Random(seed)
        return Key(''.join(str(rng.randint(0, 1)) for _ in range(key_size)))
...
class HashingAlgorithm:
    ...
    def con_hexstr_to_bin(self,key):
        ...
        Returns:
            [str]: [binary form]
        """
        return bin(int(key, base=16))[2:].zfill(len(key) * 4)
